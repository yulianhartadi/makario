...
<div class="panel">
    <div class="tab-title clearfix">
        ...
        <div class="pull-right p10 mr10 custom-toolbar">
            <?php
            ...
            $gantt_view_dropdown = array(
                array("id" => "Day", "text" => lang("days_view")),
                array("id" => "Week", "text" => lang("weeks_view")),
                array("id" => "Month", "text" => lang("months_view"))
            );
            ...
            echo form_input(array(
                ...
                "value" => get_cookie("gantt_view_of_user_" . $this->login_user->id) ? get_cookie("gantt_view_of_user_" . $this->login_user->id) : "Day"
            ));
            ?>
        </div>
    </div>
...
echo modal_anchor(get_uri("projects/task_view"), "", array("id" => "show_task_hidden", "class" => "hide"));
?>

<script type="text/javascript">
    var loadGantt = function (group_by, id, status, projectId, scrollToLast) {
        ...
        $.ajax({
            url: "<?php echo get_uri("projects/gantt_data/"); ?>" + projectId + "/" + group_by + "/" + id + "/" + status,
            type: 'POST',
            dataType: 'json',
            success: function (result) {
                appLoader.hide();
                if (!result.length) {
                    $("#gantt-chart").html("<div class='text-off text-center' style='padding: 41px;'><?php echo lang("no_result_found"); ?></div>");
                    return;
                }

                $("#gantt-chart").html("");

                var viewMode = "<?php echo get_cookie("gantt_view_of_user_" . $this->login_user->id) ? get_cookie("gantt_view_of_user_" . $this->login_user->id) : 'Day'; ?>";
                ...
                var gantt = new Gantt("#gantt-chart", result, {
                    ...
                    on_click: function (task) {
                        if (task.dependencies.length) {
                            ...
                            $("#show_task_hidden").attr("data-title", "<?php echo lang('task_info') . " #" ?>" + task.id);
                    ...
                    custom_popup_html: function (task) {
                        ...
                        if (daysCount) {
                            if (daysCount === 1) {
                                daysCount = daysCount + " <?php echo lang("day"); ?>";
                            } else {
                                daysCount = daysCount + " <?php echo lang("days"); ?>";
                            }
                        }
                        ...
                        return `
                            <div class="gantt-task-popup">
                                ...
                                <div><strong><?php echo lang("start_date"); ?>: </strong> ${startDate}</div>
                                <div><strong><?php echo lang("deadline"); ?>: </strong> ${endDate}</div>
                                <div><strong><?php echo lang("total"); ?>: </strong> ${daysCount}</div>
                            </div>
                        `;
                    }
                });
                ...
                $ganttView.on("change", function () {
                    ...
                    //save cookie
                    setCookie("gantt_view_of_user_<?php echo $this->login_user->id; ?>", type);
                });
    ...
    window.reloadGantt = function (scrollToLast) {
        ...
        loadGantt(group_by, id, status, projectId, scrollToLast);
    }
